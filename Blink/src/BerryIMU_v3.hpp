...
class BerryIMU_v3
{
  public:
    ...
    uint8_t cs = 10; //LSM6DSL Chip Select


  private:
    uint8_t readSPIReg(uint8_t ss, uint8_t reg);
    void readSPIRegs(uint8_t ss, uint8_t reg, uint8_t count, uint8_t dest[]);
    void writeSPIReg(uint8_t ss, uint8_t reg, uint8_t value);
