...
SPISettings LSM6DSLSPI(10000000, MSBFIRST, SPI_MODE3);
...
//   writeTo(LSM6DSL_ADDRESS, LSM6DSL_CTRL1_XL, 0b10011111);      // ODR 3.33 kHz, +/- 8g , BW = 400hz
  writeSPIReg(cs, LSM6DSL_CTRL1_XL, 0b10101111);
  //writeSPIReg(cs, LSM6DSL_CTRL6_C, 0b00000000);
...
//   writeTo(LSM6DSL_ADDRESS, LSM6DSL_CTRL8_XL, 0b11001000);      // Low pass filter enabled, BW9, composite filter
  writeSPIReg(cs, LSM6DSL_CTRL8_XL, 0b11001000);
...
//   writeTo(LSM6DSL_ADDRESS, LSM6DSL_CTRL3_C, 0b01000100);       // Enable Block Data update, increment during multi byte read
  writeSPIReg(cs, LSM6DSL_CTRL3_C, 0b01000100);
  //**********************************************************************************************************************************************************
...
//   writeTo(LSM6DSL_ADDRESS, LSM6DSL_CTRL2_G, 0b10011100);       // ODR 3.3 kHz, 2000 dps
  writeSPIReg(cs, LSM6DSL_CTRL2_G, 0b10011100);
...
//   readFrom(LSM6DSL_ADDRESS, LSM6DSL_OUT_X_L_XL, 6, buff);
  readSPIRegs(cs, LSM6DSL_OUT_X_L_XL, 6, buff);
...
//   readFrom(LSM6DSL_ADDRESS, LSM6DSL_OUT_X_L_G, 6, buff);
  readSPIRegs(cs, LSM6DSL_OUT_X_L_G, 6, buff);
...
uint8_t BerryIMU_v3::readSPIReg(uint8_t ss, uint8_t reg) {
    SPI.beginTransaction(LSM6DSLSPI);
...
void BerryIMU_v3::readSPIRegs(uint8_t ss, uint8_t reg, uint8_t count, uint8_t dest[]) {
    SPI.beginTransaction(LSM6DSLSPI);
    digitalWriteFast(ss, 0);
    SPI.transfer(reg | 0x80);
...
void BerryIMU_v3::writeSPIReg(uint8_t ss, uint8_t reg, uint8_t value) {
    SPI.beginTransaction(LSM6DSLSPI);
    digitalWriteFast(ss, 0);
    SPI.transfer(reg);
    SPI.transfer(value);
